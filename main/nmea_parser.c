...
{
   //ESP_LOGI("NMEA-GGA:","Item: %u Data: %s",esp_gps->item_num,esp_gps->item_str);
...
static void parse_rmc(esp_gps_t *esp_gps)
{
   ...
   // ESP_LOGI("NMEA-RMC:","Item: %u Data: %s",esp_gps->item_num,esp_gps->item_str);
...
{
 //   ESP_LOGI("NMEA-GLL:","Item: %u Data: %s",esp_gps->item_num,esp_gps->item_str);
...
static void parse_zda(esp_gps_t *esp_gps)
{
   ...
   // ESP_LOGI("nmea_parser", "ZDA. itemnum: %i itemstr: %s", esp_gps->item_num, esp_gps->item_str);
...
static void parse_pair(esp_gps_t *esp_gps)
{
    //I (841761) nmea_parser: gps_decode: $PAIR010,0,1,2409,324480*3C
  //  ESP_LOGI("nmea_parser", "PAIR. itemnum: %i itemstr: %s", esp_gps->item_num, esp_gps->item_str);
    /* Process GPVGT statement */
    //    switch (esp_gps->item_num) {
    //    case 1: /* Process true course over ground */
    //        esp_gps->parent.cog = strtof(esp_gps->item_str, NULL);
...
static esp_err_t parse_item(esp_gps_t *esp_gps)
{
    ...
    if (esp_gps->item_num == 0 && esp_gps->item_str[0] == '$')
    {
        ...
        else if (strstr(esp_gps->item_str, "PAIR"))
        {
            esp_gps->cur_statement = STATEMENT_PAIR;
        }
        else
        {
    ...
    else if (esp_gps->cur_statement == STATEMENT_PAIR)
    {
        parse_pair(esp_gps);
    }
    else
    ...
    //ESP_LOGI("GPS-Debug: ","Long: %.02f Lat: %.02f",esp_gps->parent.longitude,esp_gps->parent.latitude);
out:
    return err;
}
...
{
    //ESP_LOGI("nmea_parser", "gps_decode: %s", esp_gps->buffer);
        ...
        else if (*d == '\r')
        {
            ...
            if (esp_gps->crc == crc)
            {
                ...
                case STATEMENT_ZDA:
                    ...
                    break;
                case STATEMENT_PAIR:
                    esp_gps->parsed_statement |= 1 << STATEMENT_PAIR;
                    break;
                default:
                    break;
                }
...
static void nmea_parser_task_entry(void *arg)
{
    ...
    while (1)
    {
        if (xQueueReceive(esp_gps->event_queue, &event, pdMS_TO_TICKS(100)))
        {
            switch (event.type)
            {
            case UART_DATA:
        ...
        esp_event_loop_run(esp_gps->event_loop_hdl, pdMS_TO_TICKS(25));
    }
    vTaskDelete(NULL);
}
...
nmea_parser_handle_t nmea_parser_init(const nmea_parser_config_t *config)
{
    ...
    esp_gps->all_statements |= (1 << STATEMENT_PAIR);
    ...
    uart_flush(esp_gps->uart_port);
    /*const char cmd1[] = "AT+CGNSSPWR=1\r\n";
    const char cmd2[] = "AT+CGNSSTST=1\r\n";
    const char cmd3[] = "AT+CGNSSPORTSWITCH=0,1\r\n";
    uart_write_bytes(esp_gps->uart_port,cmd1,sizeof(cmd1));
    uart_write_bytes(esp_gps->uart_port,cmd2,sizeof(cmd2));
    uart_write_bytes(esp_gps->uart_port,cmd3,sizeof(cmd3));
    ESP_LOGI(GPS_TAG, "GPS Enable cmd send.");*/
    /* Create Event loop */
